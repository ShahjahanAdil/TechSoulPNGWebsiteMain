...
export default function Section3() {
  return (
    <div className=" mx-auto px-4 py-10 space-y-12">
      ...
      {/* Popular Collections */}
      <div>
        <h2 className="md:text-4xl text-2xl font-semibold mb-2">Popular collections to inspire you</h2>
        <p className="text-sm text-gray-500 mb-4">Explore trending themed collections and find the perfect visual.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
          {popularCollections.map((item, idx) => (
            ...
            >
              <div className="absolute bottom-2 left-2 text-white   bg-gray-500 px-2 py-1 rounded text-sm font-medium">
                {item.title}
              </div>
            </div>
          ))}
      ...
      {/* Design Trends */}
      <div>
        <h2 className="text-xl font-semibold mb-2">Keep up with design trends</h2>
        <p className="text-sm text-gray-500 mb-4">Discover the most sought-after and trendy design collections.</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5">
          {designTrends.map((trend, index) => (
            <div
              key={index}
              className={`${index % 3 !== 0 ? 'bg-[#F5E9D7]' : 'bg-[#F5F6F5]'} hover:scale-102 bg-contain bg-no-repeat transition-all duration-500 rounded-lg overflow-hidden shadow bg-center relative h-60`}
              style={{ backgroundImage: `url(${trend.src})` }}
            >
              <div className="" />
              <div
                className="absolute bottom-3 left-3 text-black z-10"
                style={{ textShadow: '0 2px 4px rgba(0,0,0,0.6)' }}
              >
                <p className="font-semibold text-sm">{trend.title}</p>
                <p className="text-xs">{trend.count}</p>
              </div>
